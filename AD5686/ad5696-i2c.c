...
static int ad5686_i2c_read(struct ad5686_state *st, u8 addr)
{
	...
	if (ret < 0){
		dev_dbg(st->dev, "ad5686_i2c_read() FAIL %d", ret);
		return ret;
	}


	dev_dbg(st->dev, "ad5686_i2c_read() %08x", be16_to_cpu(st->data[0].d16));
			    ...
			    u8 cmd, u8 addr, u16 val)
{
	...
	dev_dbg(st->dev, "ad5686_i2c_write() %08x",
				AD5686_CMD(cmd) | AD5686_ADDR(addr)| val);

	ret = i2c_master_send(i2c, &st->data[0].d8[1], 3);
	if (ret < 0){
		dev_dbg(st->dev, "ad5686_i2c_write() FAIL %d", ret);
		return ret;
	}
...
{
	dev_dbg(&i2c->dev, "ad5686_i2c_probe() %02x", i2c->addr);
